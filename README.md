<div align="center">

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&height=180&color=0:6E40C9,50:8250DF,100:A371F7&text=ARONDITE&fontColor=ffffff&fontSize=42&fontAlignY=35&desc=IT%20%7C%20Cybersecurity%20%7C%20Software%20Development&descAlignY=55&animation=fadeIn" />

### Hi, I'm **Arondite** 👋

*Building software, exploring cybersecurity, and creating interactive experiences.*

[![GitHub](https://img.shields.io/badge/GitHub-Arondith-181717?style=for-the-badge&logo=github)](https://github.com/Arondith)
[![Profile Views](https://komarev.com/ghpvc/?username=Arondith&style=for-the-badge&label=PROFILE+VIEWS)](https://github.com/Arondith)

</div>

---

## 💻 Programming Languages & Tools

<div align="center">

[![My Skills](https://skillicons.dev/icons?i=cs,unity,php,js,html,css,mysql,git,github,arduino,blender,vscode&perline=12)](https://skillicons.dev)

</div>

---

## 👨‍💻 About Me

```csharp
class Arondith
{
    string Role = "IT Student & Developer";
    string Location = "Philippines";

    string[] Focus =
    {
        "Cybersecurity",
        "Software Development",
        "Augmented Reality",
        "Web Development",
        "Networking"
    };

    string[] Technologies =
    {
        "C#", "Unity", "Vuforia",
        "PHP", "JavaScript", "HTML", "CSS",
        "SQL", "Arduino", "Git", "GitHub",
        "Blender"
    };

    string CurrentGoal =
        "Build practical projects, strengthen cybersecurity skills, and keep learning.";
}
```

---

## 🚀 Current Works

- 🥽 **KoroAR** — a mobile augmented reality museum experience built with Unity, C# and Vuforia.
- 📚 **[Schola](https://github.com/Arondith/Schola)** — a responsive study-platform prototype for organizing learning activities.
- 📊 **[GitHub Profile Analyzer](https://github.com/Arondith/your_username.github.io)** — a browser-based tool that analyzes public GitHub profiles using the GitHub REST API.
- 🧠 **[CourseRecommender](https://github.com/Arondith/CourseRecommender)** — one of my software-development projects.

---

## 🛡️ Certifications & Learning

<img src="https://img.shields.io/badge/Cisco%20Networking%20Academy-Cyber%20Threat%20Management-1BA0D7?style=for-the-badge&logo=cisco&logoColor=white" alt="Cisco Cyber Threat Management" />

- **Cyber Threat Management** — Cisco Networking Academy, 2026
- Currently strengthening skills in **cybersecurity, networking, software development, and GitHub workflows**.

---

## 📈 GitHub Stats

<div align="center">

<img height="170" src="https://github-readme-stats.vercel.app/api?username=Arondith&show_icons=true&hide_border=true&theme=tokyonight&rank_icon=github" alt="Arondith GitHub stats" />
<img height="170" src="https://github-readme-stats.vercel.app/api/top-langs/?username=Arondith&layout=compact&hide_border=true&theme=tokyonight" alt="Arondith top languages" />

<br/>

<img src="https://streak-stats.demolab.com?user=Arondith&theme=tokyonight&hide_border=true" alt="Arondith GitHub streak" />

</div>

---

## 🌐 Featured Repositories

<div align="center">

[![Schola](https://github-readme-stats.vercel.app/api/pin/?username=Arondith&repo=Schola&theme=tokyonight&hide_border=true)](https://github.com/Arondith/Schola)
[![CourseRecommender](https://github-readme-stats.vercel.app/api/pin/?username=Arondith&repo=CourseRecommender&theme=tokyonight&hide_border=true)](https://github.com/Arondith/CourseRecommender)

</div>

---

## 🤝 Connect With Me

<div align="center">

[![GitHub](https://img.shields.io/badge/GITHUB-ARONDITH-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/Arondith)
[![Repositories](https://img.shields.io/badge/VIEW-MY%20PROJECTS-6E40C9?style=for-the-badge&logo=github&logoColor=white)](https://github.com/Arondith?tab=repositories)

</div>

---

<div align="center">

### ⚡ Keep learning. Keep building. Keep improving.

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&height=100&section=footer&color=0:6E40C9,50:8250DF,100:A371F7" />

</div>
